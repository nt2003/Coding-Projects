...
There
is no need for any any need for additional classes in this module. If you need 
more
classes, 99% of the time they belong in either the wave module or the models 
module. If
you are ensure about where a new class should go, post a question on Ed 
Discussions.
...
# PRIMARY RULE: Planetoids can only access attributes in wave.py via 
# getters/setters
...
class Planetoids(GameApp):
    ...
    game started, paused, completed, etc. It keeps track of that in an 
    attribute
    called _state. For a complete description of how the states work, see the 
    specification for the method update().
    
    As a subclass of GameApp, this class has the following (non-hidden) 
    inherited
    INSTANCE ATTRIBUTES:
    ...
    This attributes are inherited. You do not need to add them. Any other 
    attributes
    ...
    #
    # Attribute _wave: the subcontroller for a single wave, which manages the 
    # game
    ...
    # Invariant: _message is a GLabel, or None if there is no message to 
    # display. It is 
    ...
    def start(self):
        ...
        creates both the title (in attribute _title) and a message (in attribute
         _message) saying that the user should press a key to play a game.
        """
        ...
        font_size= TITLE_SIZE, top= 450, left= 50)
        self._message= GLabel(text="Press 'S' to Start",\
        font_name= MESSAGE_FONT\
        , font_size= MESSAGE_SIZE, top= 320, left= 160)
    ...
    def update(self,dt):
        ...
        STATE_INACTIVE, STATE_LOADING, STATE_ACTIVE, STATE_PAUSED, and 
        STATE_CONTINUE. Each one of these does its own
        thing, and might even needs its own helper. We describe these below.
        
        STATE_INACTIVE: This is the state when the application first opens. 
        It is a paused state, waiting for the player to start the game. It 
        displays a simple message on the screen. The application remains in this
         state so long as the player never presses a key. In addition, the 
        application returns to this state when the game is over (all lives are 
        lost or all planetoids are destroyed).
        
        STATE_LOADING: This is the state creates a new wave and shows it on 
        the screen.The application switches to this state if the state was 
        STATE_INACTIVE in the previous frame, and the player pressed a key. 
        This state only lasts one animation
        frame before switching to STATE_ACTIVE.
        
        STATE_ACTIVE: This is a session of normal gameplay. The player can move 
        the ship and fire bullets. All of this should be handled inside of class
        Wave (NOT in this class). Hence the Wave class should have an update() 
        method, just
        ...
        if self._state == STATE_INACTIVE:
            ...
            font_name= MESSAGE_FONT\
            , font_size= MESSAGE_SIZE, top= 320, left= 160)
        Determinant= self._sdown == False and self.input.is_key_down('s') == \
        True
        if Determinant:
    ...
    def update_helper(self):
        """
        This is a helper for the Update Methods
        """
        if self._state == STATE_ACTIVE:
            ...
            self._title= None
            self._message=GLabel(text=("Press 'S' to Continue. You have " + \
            str(self._wave.getlives()) +" lives left"),font_name= MESSAGE_FONT\
            , font_size= (MESSAGE_SIZE/1.8), top= 360, left= 75)
            Determinant= self.input.is_key_down('s') == True
            if Determinant:
                self._state = (self._state + 1) % NUM_STATES
        self.help_help_update()
    
            
    def help_help_update(self):
        """
        This is a helper for the helper method of update
        """
        if self._state== STATE_CONTINUE:
            if self._wave.getlives()== 0:
                self._state= self._state +1
            elif self._wave.getlives() > 0 :
                self._wave._re_init_ship_()
                self._state= STATE_ACTIVE
        if self._state== STATE_COMPLETE:
            if self._wave.getship() ==None:
                self._message= GLabel(text="Do Better!", \
                font_name= MESSAGE_FONT\
                , font_size= MESSAGE_SIZE, top= 375, left= 230)
            elif self._wave.getasteroids() == []:
                self._message= GLabel(text="You Win!", \
                font_name= MESSAGE_FONT\
                , font_size= MESSAGE_SIZE, top= 375, left= 260)
    ...
    def draw(self):
        ...
        Many of the GObjects (such as the ships, planetoids, and bullets) are 
        ...
        """
        # IMPLEMENT ME
        if self._title != None and self._message != None:
            self._message.draw(self.view)
            self._title.draw(self.view)
        elif self._message != None:
            self._message.draw(self.view)
        if self._state== STATE_ACTIVE:
